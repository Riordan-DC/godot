...
inline void _normalize_weights_preserving(FixedArrayV<float, 4> &weights, unsigned int preserved_index,
...
inline void normalize_weights_preserving(FixedArrayV<float, 4> &weights, unsigned int preserved_index) {
...
/*inline void normalize_weights(FixedArrayV<float, 4> &weights) {
	float sum = 0;
...
inline void blend_texture_packed_u16(int texture_index, float target_weight,
	...
	FixedArrayV<uint8_t, 4> indices = decode_indices_from_packed_u16(encoded_indices);
	FixedArrayV<uint8_t, 4> weights = decode_weights_from_packed_u16(encoded_weights);
