...
void test_encode_weights_packed_u16() {
	...
	FixedArrayV<uint8_t, 4> decoded_weights = decode_weights_from_packed_u16(encoded_weights);
	...
	// Block tests
	{
		...
		struct L {
			static void check_weights(VoxelBufferInternal &buffer, Vector3i pos,
				...
				const FixedArrayV<uint8_t, 4> indices = decode_indices_from_packed_u16(encoded_indices);
				const FixedArrayV<uint8_t, 4> weights = decode_weights_from_packed_u16(encoded_weights);
