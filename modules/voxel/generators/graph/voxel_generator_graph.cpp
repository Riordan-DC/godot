...
void VoxelGeneratorGraph::gather_indices_and_weights(Span<const WeightOutput> weight_outputs,
		...
		VoxelBufferInternal &out_voxel_buffer, FixedArrayV<uint8_t, 4> spare_indices) {
	VOXEL_PROFILE_SCOPE();
	...
	FixedArrayV<Span<const float>, 16> buffers;
	...
	if (buffers_count <= 4) {
		...
		for (int rz = rmin.z; rz < rmax.z; ++rz) {
			for (int rx = rmin.x; rx < rmax.x; ++rx) {
				FixedArrayV<uint8_t, 4> weights;
				FixedArrayV<uint8_t, 4> indices = spare_indices;
				weights.fill(0);
				for (unsigned int oi = 0; oi < buffers_count; ++oi) {
		...
		unsigned int value_index = 0;
		FixedArrayV<uint8_t, 16> skipped_outputs;
		for (int rz = rmin.z; rz < rmax.z; ++rz) {
			for (int rx = rmin.x; rx < rmax.x; ++rx) {
				FixedArrayV<uint8_t, 4> weights;
				FixedArrayV<uint8_t, 4> indices;
...
VoxelGenerator::Result VoxelGeneratorGraph::generate_block(VoxelBlockRequest &input) {
	...
	FixedArrayV<uint8_t, 4> spare_texture_indices = runtime_ptr->spare_texture_indices;
...
VoxelGraphRuntime::CompilationResult VoxelGeneratorGraph::compile() {
	...
	{
		FixedArrayV<bool, 16> used_indices_map;
		FixedArrayV<uint8_t, 4> spare_indices;
