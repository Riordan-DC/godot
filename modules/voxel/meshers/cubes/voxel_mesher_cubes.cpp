...
void build_voxel_mesh_as_simple_cubes(
		FixedArrayV<VoxelMesherCubes::Arrays, VoxelMesherCubes::MATERIAL_COUNT> &out_arrays_per_material,
	...
	FixedArrayV<uint32_t, Vector3i::AXIS_COUNT> neighbor_offset_d_lut;
	...
	FixedArrayV<uint32_t, VoxelMesherCubes::MATERIAL_COUNT> index_offsets(0);
		...
		for (unsigned int d = min_pos[za] - VoxelMesherCubes::PADDING; d < (unsigned int)max_pos[za]; ++d) {
			...
			for (unsigned int fy = min_pos[ya]; fy < (unsigned int)max_pos[ya]; ++fy) {
				for (unsigned int fx = min_pos[xa]; fx < (unsigned int)max_pos[xa]; ++fx) {
					FixedArrayV<unsigned int, Vector3i::AXIS_COUNT> pos;
					pos[xa] = fx;
					pos[ya] = fy;
					pos[za] = d;
