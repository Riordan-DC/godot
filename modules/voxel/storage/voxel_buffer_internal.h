...
public:
	...
	void set_default_values(FixedArrayV<uint64_t, VoxelBufferInternal::MAX_CHANNELS> values);
	...
	static inline FixedArrayV<uint8_t, MAX_CHANNELS> mask_to_channels_list(
			uint8_t channels_mask, unsigned int &out_count) {
		FixedArrayV<uint8_t, VoxelBufferInternal::MAX_CHANNELS> channels;
...
private:
	...
	FixedArrayV<Channel, MAX_CHANNELS> _channels;
	...
	for (int z = 0; z < voxels.get_size().z; ++z) {
		for (int x = 0; x < voxels.get_size().x; ++x) {
			for (int y = 0; y < voxels.get_size().y; ++y) {
				...
				FixedArrayV<uint8_t, 4> indices = decode_indices_from_packed_u16(pi);
