...
Span<T> to_span(FixedArrayV<T, N> &a) {
	return Span<T>(a.data(), a.size());
}

template <typename T, unsigned int N>
Span<T> to_span(FixedArrayV<T, N> &a, unsigned int count) {
	CRASH_COND(count > a.size());
	return Span<T>(a.data(), count);
}

template <typename T, unsigned int N>
Span<const T> to_span_const(const FixedArrayV<T, N> &a, unsigned int count) {
	CRASH_COND(count > a.size());
	return Span<const T>(a.data(), count);
}

template <typename T, unsigned int N>
Span<const T> to_span_const(const FixedArrayV<T, N> &a) {
