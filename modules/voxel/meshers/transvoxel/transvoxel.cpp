...
inline uint32_t pack_bytes(const FixedArrayV<uint8_t, 4> &a) {
	return (a[0] | (a[1] << 8) | (a[2] << 16) | (a[3] << 24));
}

void add_texture_data(std::vector<Vector2> &uv, unsigned int packed_indices,
		FixedArrayV<uint8_t, MAX_TEXTURE_BLENDS> weights) {
	struct IntUV {
		uint32_t x;
		uint32_t y;
	};
...
struct CellTextureDatas {
	...
	FixedArrayV<uint8_t, MAX_TEXTURE_BLENDS> indices;
	FixedArrayV<FixedArrayV<uint8_t, MAX_TEXTURE_BLENDS>, NVoxels> weights;
};

template <unsigned int NVoxels, typename WeightSampler_T>
CellTextureDatas<NVoxels> select_textures(const FixedArrayV<unsigned int, NVoxels> &voxel_indices,
	...
	FixedArrayV<FixedArrayV<uint8_t, MAX_TEXTURES>, NVoxels> cell_texture_weights_temp;
	FixedArrayV<IndexAndWeight, MAX_TEXTURES> indexed_weight_sums;
	...
	for (unsigned int ci = 0; ci < voxel_indices.size(); ++ci) {
		...
		const FixedArrayV<uint8_t, 4> indices = decode_indices_from_packed_u16(indices_data[data_index]);
		const FixedArrayV<uint8_t, 4> weights = weights_sampler.get_weights(data_index);

		FixedArrayV<uint8_t, MAX_TEXTURES> &weights_temp = cell_texture_weights_temp[ci];
		weights_temp.fill(0);
	...
	for (unsigned int ci = 0; ci < cell_texture_weights_temp.size(); ++ci) {
		//VOXEL_PROFILE_SCOPE();
		const FixedArrayV<uint8_t, MAX_TEXTURES> &src_weights = cell_texture_weights_temp[ci];
		FixedArrayV<uint8_t, 4> &dst_weights = cell_textures.weights[ci];
...
struct TextureIndicesData {
	...
	FixedArrayV<uint8_t, 4> default_indices;
...
inline void get_cell_texture_data(CellTextureDatas<NVoxels> &cell_textures,
		...
		const FixedArrayV<unsigned int, NVoxels> &voxel_indices,
	...
	for (pos.z = min_pos.z; pos.z < max_pos.z; ++pos.z) {
		for (pos.y = min_pos.y; pos.y < max_pos.y; ++pos.y) {
			...
			for (pos.x = min_pos.x; pos.x < max_pos.x; ++pos.x, data_index += block_size_with_padding.y) {
				...
				FixedArrayV<unsigned int, 8> corner_data_indices;
				...
				FixedArrayV<float, 8> cell_samples_sdf;
				...
				FixedArrayV<Vector3i, 8> padded_corner_positions;
				...
				FixedArrayV<Vector3i, 8> corner_positions;
				...
				FixedArrayV<int, 12> cell_vertex_indices(-1);
				...
				for (unsigned int vertex_index = 0; vertex_index < vertex_count; ++vertex_index) {
					...
					if (t > 0.f && t < 1.f) {
						...
						if (!present || cell_vertex_indices[vertex_index] == -1) {
							...
							if (texturing_mode == TEXTURES_BLEND_4_OVER_16) {
								const FixedArrayV<uint8_t, MAX_TEXTURE_BLENDS> weights0 = cell_textures.weights[v0];
								const FixedArrayV<uint8_t, MAX_TEXTURE_BLENDS> weights1 = cell_textures.weights[v1];
								FixedArrayV<uint8_t, MAX_TEXTURE_BLENDS> weights;
						...
						if (!present || cell_vertex_indices[vertex_index] < 0) {
							...
							if (texturing_mode == TEXTURES_BLEND_4_OVER_16) {
								const FixedArrayV<uint8_t, MAX_TEXTURE_BLENDS> weights = cell_textures.weights[vi];
								add_texture_data(output.uv, cell_textures.packed_indices, weights);
							}
						}
					}
...
void build_transition_mesh(
	...
	FixedArrayV<Vector3i, 13> cell_positions;
	...
	for (int fy = min_fpos_y; fy < max_fpos_y; fy += 2) {
		for (int fx = min_fpos_x; fx < max_fpos_x; fx += 2) {
			...
			FixedArrayV<unsigned int, 9> cell_data_indices;
			...
			FixedArrayV<float, 13> cell_samples;
			...
			FixedArrayV<Vector3, 13> cell_gradients;
			for (unsigned int i = 0; i < 9; ++i) {
			...
			for (unsigned int vertex_index = 0; vertex_index < vertex_count; ++vertex_index) {
				...
				if (t > 0.f && t < 1.f) {
					...
					if (!present || cell_vertex_indices[vertex_index] == -1) {
						...
						if (texturing_mode == TEXTURES_BLEND_4_OVER_16) {
							const FixedArrayV<uint8_t, MAX_TEXTURE_BLENDS> weights0 = cell_textures.weights[index_vertex_a];
							const FixedArrayV<uint8_t, MAX_TEXTURE_BLENDS> weights1 = cell_textures.weights[index_vertex_b];
							FixedArrayV<uint8_t, MAX_TEXTURE_BLENDS> weights;
							for (unsigned int i = 0; i < cell_textures.indices.size(); ++i) {
								weights[i] = static_cast<uint8_t>(
										clamp(Math::lerp(weights0[i], weights1[i], t1), 0.f, 255.f));
					...
					if (!present || cell_vertex_indices[vertex_index] == -1) {
						...
						if (texturing_mode == TEXTURES_BLEND_4_OVER_16) {
							const FixedArrayV<uint8_t, MAX_TEXTURE_BLENDS> weights = cell_textures.weights[cell_index];
							add_texture_data(output.uv, cell_textures.packed_indices, weights);
						}
...
struct WeightSampler3U8 {
	...
	inline FixedArrayV<uint8_t, 4> get_weights(int i) const {
		FixedArrayV<uint8_t, 4> w;
		w[0] = u8_data0[i];
		w[1] = u8_data1[i];
		w[2] = u8_data2[i];
		w[3] = 255 - (w[0] + w[1] + w[2]);
